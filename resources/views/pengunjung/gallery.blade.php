                    ...
                    <div class="col-lg-4 col-md-6 wow fadeIn" data-wow-delay="0.1s">
                        <div class="gallery-item position-relative rounded overflow-hidden">
                            @if ($image->file_path && Storage::disk('public')->exists($image->file_path))
                                <img class="img-fluid w-100" src="{{ asset('storage/' . $image->file_path) }}"
                                    alt="{{ $image->description ?? 'Gambar Galeri' }}"
                                    style="height: 250px; object-fit: cover;">
                            @else
                                <img class="img-fluid w-100" src="https://placehold.co/400x250?text=No+Image"
                                    alt="Gambar Tidak Tersedia" style="height: 250px; object-fit: cover;">
                            @endif

                            <div class="gallery-overlay">
                                {{-- Tombol "LIHAT FOTO" DI POJOK KANAN BAWAH --}}
                                @if ($image->file_path && Storage::disk('public')->exists($image->file_path))
                                    <a class="btn btn-primary btn-sm py-1 px-2 position-absolute bottom-0 end-0 m-3"
                                        href="{{ Storage::url($image->file_path) }}" data-lightbox="gallery">Lihat Foto</a>
                                @else
                                    <button class="btn btn-primary btn-sm py-1 px-2 position-absolute bottom-0 end-0 m-3"
                                        disabled>Lihat Foto</button>
                                @endif
